 ...
 */
async function editArticleTags(req, res, next) {
    try {
        const {
            tags,
        } = req?.body;
        ...
        await ArticleService.updateArticleTags(articleId, tags);

        return res.status(200).json({
            success: true,
            message: "Article's tags changed successfully",
        });
    } catch (err) {
        next(err);
    }
...
export default editArticleTags;
